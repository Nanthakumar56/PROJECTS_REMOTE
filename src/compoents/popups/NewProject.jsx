...
const NewProject = ({ onClose }) => {
  ...
  const handleSearchSup = (event) => {
    setSearchTermSup(event.target.value);
  ...
  const validateForm = (formData, milestonesData) => {
    ...
    if (!milestonesData || milestonesData.length < 3) {
      errors.milestones = "At least three milestones are required";
    }
  ...
  const handleRemoveMilestone = (indexToRemove) => {
    setMilestonesData((prevMilestones) => {
      const updatedMilestones = prevMilestones.filter(
        (_, i) => i !== indexToRemove
      );

      return updatedMilestones.map((milestone, index) => ({
        ...milestone,
        step: index + 1,
      }));
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const submittedData = {
      ...
      userIds: [selectedManager?.userid || "", selectedSup?.userid || ""],
    ...
    const errors = validateForm(submittedData, milestonesData);
    ...
    try {
      const response = await fetch(
        "http://localhost:5858/projects/createProject",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        setSuccessPopupTitle("New Project");
        setSuccessPopupMsg("Project created successfully.");
        setShowSuccessPopup(true);
      } else {
        setErrorPopupTitle("New Project");
    ...
    >
      <form className="bg-white rounded-lg px-3 lg:!px-4 xl:!px-6 py-3 lg:!py-4 xl:!py-5 w-[60vw] relative">
        <div className="flex justify-between">
          <p className="text-xs lg:text-sm font-semibold">Create New Project</p>
          <button
            onClick={onClose}
          ...
          <div className="h-[320px] lg:h-[440px] overflow-auto mb-8 px-2 mt-4">
            <div className="w-full">
              <div className="flex items-center gap-x-4">
                ...
                <p
                  className={`text-[8px] lg:!text-[10px] mb-0 text-red-600 ${
                    errors && errors.milestones ? "" : "hidden"
                  }`}
                >
                  At least three milestones should be defined
                </p>
              </div>
                ...
                <div>
                  <ul>
                    {milestonesData
                      ...
                      .map((milestone, index) => (
                        <li key={index} className="py-1 px-3 border-b my-1">
                          <div className="flex justify-between items-start">
                            <div className="flex items-start gap-x-2">
                              <p className="text-[10px] lg:text-xs text-gray-500 mb-0 mt-1">
                        ...
                        ${
                          milestone.status === "Completed"
                            ...
                            <RiCloseCircleFill
                              ...
                              className="text-base lg:text-lg text-red-500 cursor-pointer"
                            />
                          </div>
                        </li>
                      ))}
      ...
      {showNewMilestone && (
        <NewMilestoneProject
          ...
          setMilestoneData={setMilestonesData}
        />
      )}
    </div>
  );
